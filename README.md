...
flowchart TD
    A["📥 下载工具"] --> B{"两种用法"}
    B -->|"双击 .command"| C["终端一步步问你"]
    B -->|"丢给 AI 助手"| D["agent 读 AGENTS.md 代劳"]

    C --> E["🔍 扫描微信目录<br/>（此步绝不碰任何文件）"]
    D --> E
    E --> F["📊 告诉你：微信占了多少<br/>哪个月最肥、多少个文件"]
    F --> G["❓ 问你两件事<br/>① 保留最近几个月？（默认 3）<br/>② 文件搬到哪里？"]
    G --> H["👀 预演：列出会搬走什么"]
    H --> I{"你确认吗？"}
    I -->|"取消"| X["退出，一个文件都没动过"]
    I -->|"确认"| J["🚚 移动文件<br/><b>是移动，不是拷贝</b>"]

    J --> K["📁 按类型分好，放进<br/>⚠️ 待清理_20260814/"]
    K --> L["💡 注意：这时磁盘空间还没变<br/>文件只是换了个位置"]
    L --> M["👤 你自己花五分钟挑"]
    ...
    O --> P{"空间释放了吗？"}
    P -->|"是"| Q["🎉 搞定"]
    P -->|"没有！"| R["运行 --check-space<br/>八成是 Time Machine 本地快照<br/>攥着已删文件不放"]
    R --> Q
...
如果是拷贝，微信那边还留着一份，磁盘占用**直接翻倍**，你删掉新的那份等于白折腾。
移动是在同一块硬盘上改指针，**瞬间完成、不产生副本**，微信那边的月份文件夹直接清空。

**② 搬完的那一刻，可用空间是不变的**

文件还在，只是从微信目录挪到了你的文件夹。**真正的空间释放发生在你按下删除的那一刻。**
不明白这点的人会以为"整理完空间没变，这工具没用"。

**③ 删完还是没释放？那是 macOS 的锅，不是你的**

Time Machine **本地快照**会攥着已删文件不放。实测：删 35GB 只回血 29GB、删 17GB
只回血 5.7GB、某台机器上快照一个人锁了 **620GB**。跑 `--check-space` 一查便知。
...
```mermaid
flowchart LR
    A["⏰ 每 3 个月<br/>自动运行"] --> B["搬出又攒够<br/>3 个月的文件"]
    B --> C["📋 在「提醒事项」<br/>建一条待办"]
    C --> D["你有空时处理<br/>没空就放着"]
    D --> E["⚠️ 待清理_日期/<br/>（每批独立，不会混）"]
    E --> F["挑出要留的<br/>→ ✅ 我要留的"]
    E --> G["整批删掉"]
    F --> A
    G --> A
...
因为三个月后新搬出来的文件，会跟你上次没清完的混在一起 —— 你分不清哪些是旧的，
更糟的是**同名文件会互相覆盖**，你上次特意留下的可能被新的冲掉。
所以每次运行自成一批 `⚠️ 待清理_日期/`，而 `✅ 我要留的/` 是你的地盘，**工具永远只读不写**。
