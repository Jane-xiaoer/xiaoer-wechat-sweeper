...
<h3 id="windows-怎么用">方式一（Windows）：双击 .bat</h3>
...
```bash
python3 wechat_cleaner.py            # 交互式问答（Windows 上把 python3 换成 python）
...
3. **`✅ 我要留的` 是你的地盘** —— 工具只读不写，永远不会覆盖或移动里面的东西
...
| | 怎么做 | 适合谁 |
|---|---|---|
| **双击** | 右键 `小耳微信清扫器·面板.command` →「打开」 | 大多数人 |
...
视频基本可以全删，文档挑几个想留的拖进 `✅ 我要留的`，剩下的按分类夹整个删掉。
